...
export default class TimecopView {
  ...
  showLoadedPackages () {
    let count, time
    let packages = atom.packages.getLoadedPackages().filter(pack => pack.getType() !== 'theme')
    {time, count, packages} = this.getSlowPackages(packages, 'loadTime')
  ...
  showActivePackages () {
    let count, time
    let packages = atom.packages.getActivePackages().filter(pack => pack.getType() !== 'theme');
    {time, count, packages} = this.getSlowPackages(packages, 'activateTime')
